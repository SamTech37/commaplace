...
(function () {
  ...
  function syncButtons() {
    for (var i = 0; i < btns.length; i++) {
      btns[i].setAttribute(
        "aria-pressed",
        btns[i].getAttribute("data-val") === mode ? "true" : "false"
      );
    }
  }
  ...
  function select(next) {
    ...
    // Commit mode/buttons/localStorage synchronously so a rapid second click
  ...
  for (var i = 0; i < btns.length; i++) {
    (function (b) {
      b.addEventListener("click", function () {
        select(b.getAttribute("data-val"));
      });
    })(btns[i]);
  }
