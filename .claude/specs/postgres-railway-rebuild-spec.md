...
Rip out SQLite and rebuild commonplace's backend on Postgres + Render, with
...
**In:**
- Postgres as the database engine (Render-hosted, single provider for both
...
worker. A container platform (Render) runs it unchanged. True
...
**Provider: Render, single provider for DB + compute.**
Render keeps the app and its managed Postgres in one platform, which keeps
the deployment simple and avoids cross-provider network latency on every
query — this app does many small synchronous DB round trips per page render,
so that latency matters more than squeezing out a more theoretical
...
- **Connection pooling**: Render Postgres + a long-lived container means
